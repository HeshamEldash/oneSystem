...
class UpdateObjectApiMixin:
    ...
    def get_object(self):
        assert self.look_up is not None, (
            "look_up field should be set as a string")
        pk = self.request.query_params.get(self.look_up)

        assert self.look_up_model is not None, (
            "look_up_model field should be set")
...
class AddTelephoneNumberApi(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        new_num = add_telephone_number(
            owner_id=owner_id, owner_type=owner_type, **serializer.validated_data)
...
class UpdateTelephoneNumberApi(APIView):
    ...
    def put(self, request, *args, **kwargs):
        telephone_id = self.request.query_params.get("telephone_id")
        owner_type = self.request.query_params.get("owner_type", None)

        serializer = self.InputSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        new_num = update_telephone_number(
            telephone_id=telephone_id, owner_type=owner_type, **serializer.validated_data)
        print(new_num)
        return Response(status=status.HTTP_200_OK, data=TelephoneNumberSerializer(new_num).data)
...
class BranchTelephoneUpdateApi(APIView, UpdateObjectApiMixin, ProviderActionPermissionMixin):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        provider = self.get_provider()
        self.check_object_permissions(request, provider)
        updated_branch = BranchService().update_telephone_numbers(instance, **serializer.validated_data)
...
class BranchUpdateApi(APIView, UpdateObjectApiMixin, ProviderActionPermissionMixin):
    ...
    class InputSerializer(serializers.Serializer):
        ...
        # telephone_numbers = TelephoneNumberSerializer(source='branchtelephonenumbers_set', many=True, required=False)
        branchaddress = AddressSerializer( required=False)
    ...
    def patch(self, request, *args, **kwargs):
        ...
        provider = self.get_provider()
        self.check_object_permissions(request, provider)
        updated_branch = BranchService().update(instance, **serializer.validated_data)
        return Response(status=status.HTTP_200_OK, data=BranchDetailSerializer(updated_branch).data)
...
class ProviderUpdateApi(APIView, UpdateObjectApiMixin):
    ...
    class InputSerializer(serializers.Serializer):
        ...
        owner = serializers.SlugRelatedField(
            slug_field="pk", queryset=Account.objects.all(), required=False)
...
class EmploymentUpdateApi(APIView, UpdateObjectApiMixin, ProviderActionPermissionMixin):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        provider = self.get_provider()
        self.check_object_permissions(request, provider)
        updated_employment = update_employment(
...
class EmploymentCreateApi(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        provider_instance = get_object_or_404(
            Provider, pk=serializer.validated_data["provider"])
...
class PatientProfileProviderDetailApi(APIView, ProviderActionPermissionMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        provider = self.get_provider()
        patient = PatientProviderService(
            provider).get_patient_profile(patient_pk=patient_pk)

        self.check_object_permissions(request, provider)
        serializer = NewPatientProfileDetailSerializer(patient)
...
class PatientProfileProviderCreateApi(APIView, ProviderActionPermissionMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        provider = self.get_provider()
        self.check_object_permissions(request, provider)
        patient_obj = PatientProviderService(
            provider=provider).create_patient_and_registration(**serializer.validated_data)
...
class PatientProfileProviderUpdateApi(APIView, UpdateObjectApiMixin, ProviderActionPermissionMixin):
    ...
    class InputSerializer(serializers.Serializer):
        ...
        telephone_numbers = TelephoneNumberSerializer(
            source='patienttelephonenumbers_set', many=True, required=False)
        address = AddressSerializer(source='patientaddress', required=False)
    ...
    def patch(self, request, *args, **kwargs):
        ...
        provider = self.get_provider()
        self.check_object_permissions(request, provider)
        updated_patient = PatientProviderService(provider=provider).update_patient(
            instance, **serializer.validated_data)
...
class ProviderRegistrationDetailApi(APIView, ProviderActionPermissionMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        provider = self.get_provider()
        self.check_object_permissions(request, provider)
        registration = PatientProviderService(
            provider=provider).get_registration(patient_id=patient_id)
...
class ProviderRegistrationListApi(APIView, ProviderActionPermissionMixin):
    ...
    def get(self, request):
        ...
        provider = self.get_provider()
        self.check_object_permissions(request, provider)
        registration_list = get_provider_registration_list(
...
class ProviderRegistrationCreateApi(APIView, ProviderActionPermissionMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        self.check_object_permissions(request, provider)
        new_registration = PatientProviderService(
