...
import { dbApiDataHandler, readFromDb } from "../../api/dbFunc.jsx";
...
const getClinicData = async ({ provider_id: provider_id }) => {
  let response = await axios.get(`${APIENDPOINT}/appointments/clinics/`, {
    headers: {
      "Content-type": "application/json",
      Authorization:
        "Bearer " +
...
const deleteClinic = async (clinic_id) => {
  await axios
    .delete(`${APIENDPOINT}/appointments/clinic/`, {
      headers: {
        "Content-type": "application/json",
        Authorization:
          "Bearer " +
...
const bookAppointment = async (slot_id, patient_id, presentation) => {
  let response = await fetch(
    `${APIENDPOINT}/appointments/appointment/?` +
      new URLSearchParams({ slot_id: slot_id }),
    {
      method: "POST",
      headers: {
        "Content-type": "application/json",
