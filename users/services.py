...
class BranchService:
    ...
    def create( provider: Provider, branch_name: str, unit_number: str,
        ...
        address = BranchAddress.objects.create(unit_number=unit_number, first_line=first_line,
                                second_line=second_line, city=city, governorate=governorate, branch = branch
                                )

        telephone_number = telephone_create(
            telephone_number=telephone_number
        )

        branch.save()
        
        branch.phone_nums.add(telephone_number)
...
def check_patient_clinican_relationship(provider:Provider, staff:Staff, patient:Patient):    
    ...
    providers_querySet = Provider.objects.filter(employment__staff = staff, registration__patient = patient)
...
def check_manager_staff_relationship(staff:Staff, manager:Account):
        ...
        if not hasattr(manager, "staff"):  return False   

        staff_providers = Provider.objects.filter(employment__staff = staff)
        manager_provider = Provider.objects.filter(employment__staff = manager.staff, employment__employment_role="MG")
...
class StaffService:
    ...
    def staff_create(email:str, password:str,first_name:str,middle_names:str, last_name:str,professional_number:str,staff_role:str,telephone_number:str ):
        ...
        telephone = telephone_create(telephone_number=telephone_number)
...
def create_employment(staff:int, provider:int, salary:float, employment_role:str):
    ...
    try:
        emp = Employment.objects.create(provider=provider, staff=staff, employment_role=employment_role, salary=salary)
...
def update_employment(instance, **kwargs ):
    ...
    instance.employment_role = kwargs.get('employment_role', instance.employment_role)
...
def get_provider_employment_list(provider_pk:int):   
    ...
    return provider.employment_set.all()
    ...
    def create_patient_profile( self,
        ...
        if created:
            PatientTelephoneNumbers.objects.create(
                telephone_number =telephone_number, patient= patient_obj
            )
